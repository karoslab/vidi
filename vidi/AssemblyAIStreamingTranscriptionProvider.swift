...
final class AssemblyAIStreamingTranscriptionProvider: BuddyTranscriptionProvider {
    ...
    private static let tokenProxyURL = VidiConfig.workerBaseURL + "/transcribe-token"

    let displayName = "AssemblyAI"
    let requiresSpeechRecognitionPermission = false

    /// Only configured once the Worker proxy URL has been patched in —
    /// lets the provider factory fall back gracefully to another provider
    /// while the placeholder URL is still in VidiConfig.
    var isConfigured: Bool { VidiConfig.isWorkerConfigured }
    var unavailableExplanation: String? {
        isConfigured ? nil : "Worker proxy is not configured yet (VidiConfig.workerBaseURL still has the placeholder subdomain)."
    }
    ...
    private func fetchTemporaryToken() async throws -> String {
        guard let tokenRequestURL = URL(string: Self.tokenProxyURL) else {
            throw AssemblyAIStreamingTranscriptionProviderError(
                message: "Invalid transcribe-token proxy URL: \(Self.tokenProxyURL)"
            )
        }
