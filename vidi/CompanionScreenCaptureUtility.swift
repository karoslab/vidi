...
enum CompanionScreenCaptureUtility {
    ...
    static func captureAllScreensAsJPEG() async throws -> [CompanionScreenCapture] {
        #if DEBUG
        // VP Lab bisect gate (CoreAudio dig, Day 1): when the SCK/vision-capture
        // row is disabled, throw BEFORE touching ScreenCaptureKit so no SCK
        // session ever spins up during a VP soak. Every caller aborts through its
        // existing capture-error path — genuinely not-started, not merely hidden.
        if VPLab.isDisabled(.sckVisionCapture) {
            vlog("🧪 VPLab: SCK/vision capture DISABLED — refusing capture")
            throw NSError(domain: "CompanionScreenCapture", code: -100,
                          userInfo: [NSLocalizedDescriptionKey: "SCK/vision capture disabled by VP Lab"])
        }
        #endif
