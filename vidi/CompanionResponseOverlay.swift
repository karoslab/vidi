...
final class CompanionResponseOverlayManager {
    ...
    private func createOverlayPanelIfNeeded() {
        ...
        // `.fixedSize()` is load-bearing: it pins the hosting view to the
        // SwiftUI content's IDEAL size. Without it, the hosting view's width is
        // unconstrained, so `fittingSize` reports the fully-compressed width
        // (~one glyph) and a matching full-height column — which rendered as a
        // full-screen-tall dark vertical stripe over the menu bar whenever a
        // chime fired with non-trivial text (fixed 2026-07-07). With fixedSize
        // the bubble hugs its text (up to the inner 300pt maxWidth, wrapping
        // taller) and reports a compact, correct size.
    ...
    private func resizePanelToFitContent() {
        ...
        let newWidth = min(max(fittingSize.width, 1), overlayMaxWidth)
        // Defense-in-depth against the full-height-stripe regression: even if a
        // future change lets the hosting view collapse horizontally again, the
        // bubble must never paint past the visible height of the screen it sits
        // on (which would cover the menu bar). Clamp to the cursor screen's
        // visible height with a margin.
        let visibleHeight = screenContainingPoint(NSEvent.mouseLocation)?.visibleFrame.height ?? 600
        let newHeight = min(fittingSize.height, max(visibleHeight - 40, 40))
...
private struct CompanionResponseOverlayView: View {
    ...
    var body: some View {
        if viewModel.isShowingResponse {
            // A compact, OPAQUE reading bubble (Phase 11E): response text stays
            // high-contrast and never blurs/darkens the user's real desktop —
            // it is a small card, not a large glass field. Reads in light/dark.
            Text(viewModel.streamingResponseText.isEmpty ? "..." : viewModel.streamingResponseText)
                ...
                .background(
                    RoundedRectangle(cornerRadius: 12, style: .continuous)
                        .fill(VC.Colors.surfaceReadingElevated)
                        .overlay(
                            RoundedRectangle(cornerRadius: 12, style: .continuous)
                                .stroke(VC.Colors.hairline, lineWidth: 0.8)
                        )
                        .shadow(color: VC.Colors.glassShadow, radius: 16, x: 0, y: 8)
                )
        }
    }
}
