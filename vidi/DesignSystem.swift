...
//  vidi
//
//  Shared UI infrastructure: the button-style system, cursor/tooltip AppKit
//  bridges, and the general-purpose `Color(hex:)` initializer.
//
//  Phase 11A (Frost Liquid Glass) consolidation: this file no longer carries a
//  competing visual palette. Its semantic color anchors (canvas/surface/text/
//  border/action/status) now DELEGATE to the ONE semantic layer in
//  `VidiCurrentTheme.swift` (`VC.Colors.*`), so `DS` and `VC` are a single
//  adaptive light/dark system, not two. The only colors kept local are the
//  deliberately-distinct screen-overlay cursor identity (blue cursor, kept
//  distinct from the in-app accent) and the floating/waveform identity hues.
//
...
enum DS {
    ...
    enum Colors {
        ...
        // These delegate to the ONE Frost semantic layer (Phase 11A) so DS and
        // VC are a single adaptive light/dark system, not two competing ones.

        /// The deepest background — used for the main app window fill.
        static let background = VC.Colors.canvasSolid

        /// First elevation layer — reading surface.
        static let surface1 = VC.Colors.surfaceReading

        /// Second elevation layer — elevated reading surface.
        static let surface2 = VC.Colors.surfaceReadingElevated

        /// Third elevation layer — used for hover backgrounds on interactive elements.
        static let surface3 = VC.Colors.glassFillStrong

        /// Fourth elevation layer — used for active/pressed states on interactive elements.
        static let surface4 = VC.Colors.glassFillStrong

        // ── Borders ──────────────────────────────────────────────────

        /// Subtle border — used for card outlines, dividers, input field borders.
        static let borderSubtle = VC.Colors.hairline

        /// Strong border — used for focused inputs, hovered card outlines.
        static let borderStrong = VC.Colors.glassBorder

        // ── Text ─────────────────────────────────────────────────────

        /// Primary text — main body text, titles, headings.
        static let textPrimary = VC.Colors.textPrimary

        /// Secondary text — descriptions, hints, muted labels.
        static let textSecondary = VC.Colors.textSecondary

        /// Tertiary text — very muted, used for section labels, timestamps, disabled text.
        static let textTertiary = VC.Colors.textTertiary

        /// Text used on top of `accent`/`accentHover` (now the Frost
        /// `actionSecondary`/`focusRing` tokens, not the old #2563eb blue —
        /// see the Phase 11A delegation above). White FAILS WCAG AA on
        /// `actionSecondary` (3.1:1 light / 2.06:1 dark; needs 4.5:1) — this
        /// is only correct as-is because `DSPrimaryButtonStyle`, its one
        /// consumer, has no call sites in the app (dead code). Do not reuse
        /// this token for a live white-on-accent button; use
        /// `VC.Colors.textOnAccent` (dark ink) instead, which measures
        /// 5.72:1 / 8.62:1 on the same fills.
        ...
        /// Accent fill — the Frost secondary (blue) action token.
        static let accent = VC.Colors.actionSecondary

        /// Accent hover — the Frost focus-ring blue (deeper than accent).
        static let accentHover = VC.Colors.focusRing

        /// Accent text — used for accent-colored text and icons.
        static let accentText = VC.Colors.actionSecondary

        /// Very subtle accent tint — used for selected item backgrounds.
        static let accentSubtle = VC.Colors.actionSecondary.opacity(0.10)

        // ── Semantic Colors ──────────────────────────────────────────

        /// Destructive/error actions — delegates to the Frost danger token.
        static let destructive = VC.Colors.stateDanger

        /// Destructive hover state.
        static let destructiveHover = VC.Colors.stateDanger

        /// Destructive used for text.
        static let destructiveText = VC.Colors.stateDanger

        /// Success — checkmarks, granted status, completion indicators.
        static let success = VC.Colors.stateSuccess

        /// Warning — caution messages, manual verification failure explanations.
        static let warning = VC.Colors.stateWarning

        /// Warning text.
        static let warningText = VC.Colors.stateWarning

        /// Info/feature highlight — the Frost secondary (blue) token.
        static let info = VC.Colors.actionSecondary
