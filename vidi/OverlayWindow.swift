...
struct BlueCursorView: View {
    ...
    private let fullWelcomeMessage = "hey! i'm vidi"
    ...
    var body: some View {
        ZStack {
            ...
            // Pig cursor 🐷 — shown when idle or while TTS is playing (responding).
            // All three states (pig, waveform, spinner) stay in the view tree
            // permanently and cross-fade via opacity so SwiftUI doesn't remove/re-insert
            // them (which caused a visible cursor "pop").
            //
            // The pig keeps every existing flight behavior: it rides the same
            // frame-by-frame bezier arc (`cursorPosition`), swoops via `buddyFlightScale`,
            // and fades transiently via `cursorOpacity`. It is deliberately NOT rotated
            // to face its direction of travel (as the old triangle was) — a tilting pig
            // reads as broken, so the emoji stays upright.
            //
            ...
            Text("🐷")
                .font(.system(size: 19))
                .shadow(color: Color.black.opacity(0.35), radius: 3 + (buddyFlightScale - 1.0) * 8, x: 0, y: 1)
            ...
            // Blue spinner — shown while the AI is processing (transcription + the model + waiting for TTS)
...
class OverlayWindowManager {
    ...
    func showOverlay(onScreens screens: [NSScreen], companionManager: CompanionManager) {
        #if DEBUG
        // VP Lab bisect gate (CoreAudio dig, Day 1): when the overlay-rendering
        // row is disabled, never create the full-screen NSHostingView overlay
        // windows (a candidate main-thread/IO-starvation contributor to the VP
        // death). All 9 CompanionManager call sites funnel through here, so this
        // one gate genuinely keeps the overlay not-rendered.
        if VPLab.isDisabled(.overlayRendering) {
            vlog("🧪 VPLab: overlay rendering DISABLED — not showing overlay")
            return
        }
        #endif
